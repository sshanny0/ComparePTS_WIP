...
Public Class WeekReport
    Public Sub LoadReport()
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            query &= "SELECT '" & DT_WEEK_FROM.Text & "' AS date_from, '" & DT_WEEK_TO.Text & "' AS date_to, * "
            query &= "FROM (SELECT RANK() OVER (PARTITION BY machine_cd, line_cd, part_name ORDER BY a.created_at DESC) AS rk, "
            ...
            query &= "WHERE date_input BETWEEN '" & DT_WEEK_FROM.Text & "' AND '" & DT_WEEK_TO.Text & "' AND line_cd = '" & T_LINE_CD.Text & "') t "
            query &= "WHERE rk = 1"
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            query &= "WHERE date BETWEEN '" & DT_WEEK_FROM.Text & "' AND '" & DT_WEEK_TO.Text & "' AND line_cd = '" & T_LINE_CD.Text & "') t "
            query &= "WHERE rk = 2"
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            Dim query As String = String.Empty
            query &= "SELECT '" & DT_WEEK_FROM.Text & "' AS date_from, '" & DT_WEEK_TO.Text & "' AS date_to, MAX(created_at) created_at, MAX(line_cd) line_cd, "
            query &= "MAX(line_desc) line_desc FROM mt_checked_machine_final "
            query &= "WHERE date_input BETWEEN '" & DT_WEEK_FROM.Text & "' AND '" & DT_WEEK_TO.Text & "' AND line_cd = '" & T_LINE_CD.Text & "' "
            Using cmd As New SqlCommand(query, con)
                Dim da As New SqlDataAdapter
                da.SelectCommand = cmd
                da.Fill(dt3)
        ...
        With Me.R_WEEKLY.LocalReport
            .DataSources.Clear()
            '.ReportPath = "E:\Hanny\VB\CheckSheetSystem\DailyMaintenance\Report\Weekly\WeekReport.rdlc"
            .ReportPath = Application.StartupPath + "\Report\WeekReport.rdlc"
    ...
    Private Sub WeekReport_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GetLine()
    End Sub
