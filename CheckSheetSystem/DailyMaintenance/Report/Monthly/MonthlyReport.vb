...
Public Class MonthlyReport
    Public Sub LoadReport()
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            query &= "WHERE date_input LIKE '" & DT_MONTH.Text & "%' AND line_cd = '" & T_LINE_CD.Text & "') t "
            query &= "WHERE rk = 1"
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            query &= "WHERE date LIKE '" & DT_MONTH.Text & "%' AND line_cd = '" & T_LINE_CD.Text & "') t "
            query &= "WHERE rk = 2"
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            query &= "WHERE date_input LIKE '" & DT_MONTH.Text & "%' AND line_cd = '" & T_LINE_CD.Text & "' "
            Using cmd As New SqlCommand(query, con)
                Dim da As New SqlDataAdapter
                da.SelectCommand = cmd
                da.Fill(dt3)
        ...
        Using con As New SqlConnection(My.Settings.TDF)
            ...
            Dim query As String = String.Empty
            query &= " SELECT * FROM mt_sign_approval "
            Using cmd As New SqlCommand(query, con)
                Dim da As New SqlDataAdapter
                da.SelectCommand = cmd
                da.Fill(dt4)
        ...
        With Me.R_MONTHLY.LocalReport
            .DataSources.Clear()
            '.ReportPath = "E:\Hanny\VB\CheckSheetSystem\DailyMaintenance\Report\Monthly\MonthlyReport.rdlc"
            .ReportPath = Application.StartupPath + "\Report\MonthlyReport.rdlc"
            ...
            .DataSources.Add(New Microsoft.Reporting.WinForms.ReportDataSource("DataSet4", dt4))
        End With

        Me.R_MONTHLY.RefreshReport()
    End Sub
