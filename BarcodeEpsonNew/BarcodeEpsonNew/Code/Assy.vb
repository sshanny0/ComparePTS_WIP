...
Public Class Assy
    ...
    End Sub
    Private Sub Assy_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        MainMenu.Show()
        Me.Hide()
    End Sub
    Private Sub Assy_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        cmb_assy.Select()
        ...
        queryBoard &= "WHERE BoardDesc = 'Assy'"
    ...
    Private Sub btn_print_Click(sender As Object, e As EventArgs) Handles btn_print.Click
        ...
        queryCheck &= "AND BoardNo = '" & boardNo & "' AND FORMAT(CreateDate, 'yyyy-MM') = '" & dt_prod.Value.ToString("yyyy-MM") & "' "
            ...
            Else
                ...
                For counter = Convert.ToInt32(serialcount + 1) To Convert.ToInt32(qty_end.Text)
                    ...
                    If serialno2 <= Convert.ToInt32(qty_end.Text) Then
                        ...
                        insertPCB &= "'" & result2 & "', "
                        If serialno2 < Convert.ToInt32(qty_end.Text) Then
                            insertPCB &= "'" & boardNo + strYear + strMonth + (Convert.ToInt32(serialno2) + 1).ToString("00000") & "', "
                        Else
                            insertPCB &= "NULL, "
                        End If
            ...
            ElseIf qty_end.Value <> 0 Then
                ...
                For counter = Convert.ToInt32(qty_start.Text) To Convert.ToInt32(qty_end.Text)
                    ...
                    If counter + pcbCounter <= Convert.ToInt32(qty_end.Text) Then
                        ...
                        insertPCB &= "INSERT INTO PCB_Prod_SrNo (sqno, rowsq, srno1, srno2, AssyNo, create_date) "
                        insertPCB &= "VALUES ('" & sqno + 1 & "', '" & rowsq & "', "
                        insertPCB &= "'" & boardNo + strYear + strMonth + (counter + pcbCounter).ToString("00000") & "', "
                        pcbCounter += 1
                        If counter + pcbCounter <= Convert.ToInt32(qty_end.Text) Then
                            insertPCB &= "'" & boardNo + strYear + strMonth + (counter + pcbCounter).ToString("00000") & "', "
                        Else
    ...
    Sub PrintTSC()
        ...
        If dtprint.Rows.Count > 0 Then
            For Each row As DataRow In dtprint.Rows
                ...
                Call barcode("28", "1", "128", "15", "0", "0", "1", "1", printSrno1)
                Call windowsfont("18", "17", "15", "0", "0", "0", "Times New Roman", printSrno1 & " / " & cmb_assy_value)

                'SRNO2
                If Not IsDBNull(row.Item("srno2")) Then
                    Dim printSrno2 As String = row.Item("srno2").ToString()
                    Call barcode("215", "1", "128", "15", "0", "0", "1", "1", printSrno2)
    ...
    Sub PrintCitizen()
        ...
        If ListView1.Items.Count > 0 Then
            ...
            If LabelConst.CLS_SUCCESS = connectResult Then
                ...
                connectedPrinter.SetPrintDarkness(23)
                ...
                If dtprint.Rows.Count > 0 Then
                    For Each row As DataRow In dtprint.Rows
                        ...
                        Dim printSrno1 As String = row.Item("srno1").ToString()
                        'BARCODE
                        'design.DrawTextPCFont(printSrno1, "Code 128", LabelConst.CLS_RT_NORMAL, 120, 80, 12, LabelConst.CLS_FNT_DEFAULT, 15, 0, LabelConst.CLS_PRT_RES_203, LabelConst.CLS_UNIT_MILLI)
                        ''FONT
                        'design.DrawTextPCFont(printSrno1, "Arial ", LabelConst.CLS_RT_NORMAL, 120, 50, 9, LabelConst.CLS_FNT_DEFAULT, 30, 0, LabelConst.CLS_PRT_RES_300, LabelConst.CLS_UNIT_MILLI)
                        ...
                        connectedPrinter.Preview(design, LabelConst.CLS_PRT_RES_203, LabelConst.CLS_UNIT_INCH, 300, 400)
