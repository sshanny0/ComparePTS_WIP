...
Public Class PBMQRCode
    ...
    Sub GetBoardNo()
        ...
        If dt.Rows.Count > 0 Then
            ...
            queryQty &= "SELECT TOP 1 SerialNo FROM Log_PCB_Prod_SrNo_QR "
    ...
    Sub GetLog()
        ...
        queryLog &= "SELECT AssyNo AS [Assy No], RangeCount AS Range, ProductionDate AS Date, COUNT(SrnoPrint) AS [Total Count] FROM Log_PCB_Prod_SrNo_QR
    ...
    End Sub
    Private Sub PBMQRCode_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        MainMenu.Show()
        Me.Hide()
    End Sub
    Private Sub PBMQRCode_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        cmb_assy.Select()
        ...
        queryBoard &= "WHERE BoardDesc = 'PSM'"
        ...
        For Each strInstalledPrinters In PrinterSettings.InstalledPrinters
            If strInstalledPrinters.Contains("Citizen") Then
    ...
    Private Sub btn_print_Click(sender As Object, e As EventArgs) Handles btn_print.Click
        ...
        queryCheck &= "SELECT TOP 1 SerialCount, SerialNo FROM Log_PCB_Prod_SrNo_QR "
        ...
        'CHECK SQNO
        Dim querySqno = "SELECT TOP 1 sqno FROM PCB_Prod_SrNo_QR "
        ...
        If dt3.Rows.Count > 0 Then
            ...
            Else
                For counter = Convert.ToInt32(serialcount + 1) To Convert.ToInt32(qty_end.Text)
                    ...
                    Dim result As String = boardNo + strYear + strMonth + serialno
                    Dim result2 As String = boardNo + strYear + strMonth + serialno2
                    ...
                    insertLabelHis &= "'" & result & "', CONVERT(DATETIME, '" & dt_prod.Text & "', 101), 'QR CODE')"
                    ExecuteQueryTDF(insertLabelHis)
                    'MsgBox(insertLabelHis)

                    'INSERT INTO PCB_Prod_SrNo_QR
                    For i As Integer = 0 To 4
                        If serialno2 <= Convert.ToInt32(qty_end.Text) Then
                            ...
                            insertPCB &= "INSERT INTO PCB_Prod_SrNo_QR (sqno, rowsq, srno1, srno2, srno3, srno4, srno5, AssyNo, create_date) "
                            insertPCB &= "VALUES ('" & sqno + 1 & "', '" & rowsq & "', "
                            insertPCB &= "'" & boardNo + strYear + strMonth + serialno2 & "', "
                            If serialno2 < Convert.ToInt32(qty_end.Text) Then
                                insertPCB &= "'" & boardNo + strYear + strMonth + (serialno2 + 1).ToString("00000") & "', "
                            Else
                                insertPCB &= "NULL, "
                            End If
                            If (serialno2 + 1) < Convert.ToInt32(qty_end.Text) Then
                                insertPCB &= "'" & boardNo + strYear + strMonth + (serialno2 + 2).ToString("00000") & "', "
                            Else
                                insertPCB &= "NULL, "
                            End If
                            If (serialno2 + 2) < Convert.ToInt32(qty_end.Text) Then
                                insertPCB &= "'" & boardNo + strYear + strMonth + (serialno2 + 3).ToString("00000") & "', "
                            Else
                                insertPCB &= "NULL, "
                            End If
                            If (serialno2 + 3) < Convert.ToInt32(qty_end.Text) Then
                                insertPCB &= "'" & boardNo + strYear + strMonth + (serialno2 + 4).ToString("00000") & "', "
                            Else
                                insertPCB &= "NULL, "
                            End If
                            insertPCB &= "'" & cmb_assy_value & "', CONVERT(DATETIME, '" & dt_prod.Text & "', 101))"
                            ExecuteQueryTDF(insertPCB)
                            serialno2 = (serialno2 + 5).ToString("00000")
                            rowsq = rowsq + 1
                        End If
                    Next i

                    'INSERT INTO Log_PCB_Prod_SrNo_QR
                    Dim insertLogPCB As String = String.Empty
                    insertLogPCB &= "INSERT INTO Log_PCB_Prod_SrNo_QR (ManufactureCode, AssyNo, ProductionDate, BoardNo, SerialNo, SerialCount, Srnoprint, CreateDate, "
                    ...
                    insertLogPCB &= "'" & counter & "', '" & result & "', CONVERT(DATETIME, '" & dt_prod.Text & "', 101), 'QR CODE', "
        ...
        Else
            ...
            ElseIf qty_end.Value <> 0 Then
                ...
                For counter = Convert.ToInt32(qty_start.Text) To Convert.ToInt32(qty_end.Text)
                    ...
                    insertLabelHis &= "CONVERT(DATETIME, '" & dt_prod.Text & "', 101), 'QR CODE')"
                    ExecuteQueryTDF(insertLabelHis)
                    ...
                    For i As Integer = 0 To 4
                        If (counter + pcbCounter) <= Convert.ToInt32(qty_end.Text) Then
                            ...
                            For j As Integer = 0 To 4
                                Dim currentCounterValue = counter + pcbCounter + j
                                If currentCounterValue <= Convert.ToInt32(qty_end.Text) Then
                                    insertPCB &= "'" & boardNo + strYear + strMonth + currentCounterValue.ToString("00000") & "', "
                                Else
                                    insertPCB &= "NULL, "
                                End If
                            Next j
    ...
    Sub PrintTSC()
        ...
        Dim querySqnoPrint = "SELECT TOP 1 sqno FROM PCB_Prod_SrNo_QR "
        ...
        queryPrint &= "SELECT * FROM PCB_Prod_SrNo_QR "
        ...
        If dtprint.Rows.Count > 0 Then
            For Each row As DataRow In dtprint.Rows
                ...
                'Call sendcommand("SIZE 6 mm, 6 mm")
                Call sendcommand("LIMITFEED 2")
                Call sendcommand("GAP 2 mm, 0 mm")
                Call setup("48", "7", "12", "12", "0", "2", "0")
                Call clearbuffer()
                ' SRNO1
                Dim printSrno1 As String = row.Item("srno1").ToString()
                Call sendcommand("QRCODE 10,1,L,2,M,0,M1,S1,""A" & printSrno1 & """")

                ' SRNO2
                If Not IsDBNull(row.Item("srno2")) Then
                    Dim printSrno2 As String = row.Item("srno2").ToString()
                    Call sendcommand("QRCODE 90,1,L,2,M,0,M1,S1,""A" & printSrno2 & """")
                End If

                ' SRNO3
                If Not IsDBNull(row.Item("srno3")) Then
                    Dim printSrno3 As String = row.Item("srno3").ToString()
                    Call sendcommand("QRCODE 170,1,L,2,M,0,M1,S1,""A" & printSrno3 & """")
                End If

                ' SRNO4
                If Not IsDBNull(row.Item("srno4")) Then
                    Dim printSrno4 As String = row.Item("srno4").ToString()
                    Call sendcommand("QRCODE 250,1,L,2,M,0,M1,S1,""A" & printSrno4 & """")
                End If

                ' SRNO5
                If Not IsDBNull(row.Item("srno5")) Then
                    Dim printSrno5 As String = row.Item("srno5").ToString()
                    Call sendcommand("QRCODE 330,1,L,2,M,0,M1,S1,""A" & printSrno5 & """")
                End If

                ''QR EXAMPLE STRING
                'Call sendcommand("QRCODE 88,1,H,2,M,0,M1,S1,""THE FIRMWARE HAS BEEN UPDATED""")
                Call printlabel("1", "1")
                Call closeport()
            Next
        End If
    ...
    Sub PrintCitizen()
        ...
        If ListView1.Items.Count > 0 Then
            ...
            If LabelConst.CLS_SUCCESS = connectResult Then
                ...
                connectedPrinter.SetPrintDarkness(20)
                ...
                If dtprint.Rows.Count > 0 Then
                    For Each row As DataRow In dtprint.Rows
                        ...
                        'SRNO1
                        Dim printSrno1 As String = row.Item("srno1").ToString()
                        design.DrawQRCode(printSrno1, LabelConst.CLS_ENC_CDPG_IBM850, LabelConst.CLS_RT_NORMAL, 2, LabelConst.CLS_QRCODE_EC_LEVEL_L, 2, 3)

                        ' SRNO2
                        If Not IsDBNull(row.Item("srno2")) Then
                            Dim printSrno2 As String = row.Item("srno2").ToString()
                            design.DrawQRCode(printSrno2, LabelConst.CLS_ENC_CDPG_IBM850, LabelConst.CLS_RT_NORMAL, 2, LabelConst.CLS_QRCODE_EC_LEVEL_L, 41, 3)
                        End If

                        ' SRNO3
                        If Not IsDBNull(row.Item("srno3")) Then
                            Dim printSrno3 As String = row.Item("srno3").ToString()
                            design.DrawQRCode(printSrno3, LabelConst.CLS_ENC_CDPG_IBM850, LabelConst.CLS_RT_NORMAL, 2, LabelConst.CLS_QRCODE_EC_LEVEL_L, 81, 3)
                        End If

                        ' SRNO4
                        If Not IsDBNull(row.Item("srno4")) Then
                            Dim printSrno4 As String = row.Item("srno4").ToString()
                            design.DrawQRCode(printSrno4, LabelConst.CLS_ENC_CDPG_IBM850, LabelConst.CLS_RT_NORMAL, 2, LabelConst.CLS_QRCODE_EC_LEVEL_L, 121, 3)
                        End If

                        ' SRNO5
                        If Not IsDBNull(row.Item("srno5")) Then
                            Dim printSrno5 As String = row.Item("srno5").ToString()
                            design.DrawQRCode(printSrno5, LabelConst.CLS_ENC_CDPG_IBM850, LabelConst.CLS_RT_NORMAL, 2, LabelConst.CLS_QRCODE_EC_LEVEL_L, 161, 3)
                        End If
    ...
    Private Sub PrintDocument1_PrintPage(sender As Object, e As Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        If dtprint.Rows.Count > 0 Then
            'MARGIN
            Dim qrcode1XMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode21XMargin"))
            Dim qrcode1YMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode21YMargin"))
            Dim qrcode2XMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode22XMargin"))
            Dim qrcode2YMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode22YMargin"))
            Dim qrcode3XMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode23XMargin"))
            Dim qrcode3YMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode23YMargin"))
            Dim qrcode4XMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode24XMargin"))
            Dim qrcode4YMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode24YMargin"))
            Dim qrcode5XMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode25XMargin"))
            Dim qrcode5YMarginMM As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode25YMargin"))
            'SCALLING
            Dim qrcode1XScaling As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode1XScaling"))
            Dim qrcode1YScaling As Single = Single.Parse(ConfigurationManager.AppSettings("qrcode1YScaling"))
            ...
            Dim qrcode21XMargin As Single = qrcode1XMarginMM / 25.4 * dpiX
            Dim qrcode21YMargin As Single = qrcode1YMarginMM / 25.4 * dpiY
            Dim qrcode22XMargin As Single = qrcode2XMarginMM / 25.4 * dpiX
            Dim qrcode22YMargin As Single = qrcode2YMarginMM / 25.4 * dpiY
            Dim qrcode23XMargin As Single = qrcode3XMarginMM / 25.4 * dpiX
            Dim qrcode23YMargin As Single = qrcode3YMarginMM / 25.4 * dpiY
            Dim qrcode24XMargin As Single = qrcode4XMarginMM / 25.4 * dpiX
            Dim qrcode24YMargin As Single = qrcode4YMarginMM / 25.4 * dpiY
            Dim qrcode25XMargin As Single = qrcode5XMarginMM / 25.4 * dpiX
            Dim qrcode25YMargin As Single = qrcode5YMarginMM / 25.4 * dpiY

            ' Convert thousandths of an inch to CM (SCALING)
            Dim qrcode1XScales As Single = qrcode1XScaling / 10.0F
            Dim qrcode1YScales As Single = qrcode1YScaling / 10.0F

            Dim xPos1 As Single = qrcode21XMargin
            Dim xPos2 As Single = qrcode22XMargin
            Dim xPos3 As Single = qrcode23XMargin
            Dim xPos4 As Single = qrcode24XMargin
            Dim xPos5 As Single = qrcode25XMargin
            Dim yPos As Single = qrcode25YMargin
            For Each row As DataRow In dtprint.Rows
                'SET SIZE OF THE QR CODE
                Dim scaleMatrix As New Matrix()
                scaleMatrix.Scale(qrcode1XScales, qrcode1YScales)
                e.Graphics.Transform = scaleMatrix

                ' SRNO1
                Dim printSrno1 As String = row.Item("srno1").ToString()
                Dim qrcode1 As New BarcodeWriter
                qrcode1.Format = BarcodeFormat.QR_CODE
                Dim qrBitmap1 As Bitmap = qrcode1.Write(printSrno1)
                e.Graphics.DrawImage(qrBitmap1, New PointF(xPos1, yPos))

                ' SRNO2
                If Not IsDBNull(row.Item("srno2")) Then
                    Dim printSrno2 As String = row.Item("srno2").ToString()
                    Dim qrcode2 As New BarcodeWriter
                    qrcode2.Format = BarcodeFormat.QR_CODE
                    Dim qrBitmap2 As Bitmap = qrcode2.Write(printSrno2)
                    e.Graphics.DrawImage(qrBitmap2, New PointF(xPos2, yPos))
                End If

                ' SRNO3
                If Not IsDBNull(row.Item("srno3")) Then
                    Dim printSrno3 As String = row.Item("srno3").ToString()
                    Dim qrcode3 As New BarcodeWriter
                    qrcode3.Format = BarcodeFormat.QR_CODE
                    Dim qrBitmap3 As Bitmap = qrcode3.Write(printSrno3)
                    e.Graphics.DrawImage(qrBitmap3, New PointF(xPos3, yPos))
                End If

                ' SRNO4
                If Not IsDBNull(row.Item("srno4")) Then
                    Dim printSrno4 As String = row.Item("srno4").ToString()
                    Dim qrcode4 As New BarcodeWriter
                    qrcode4.Format = BarcodeFormat.QR_CODE
                    Dim qrBitmap4 As Bitmap = qrcode4.Write(printSrno4)
                    e.Graphics.DrawImage(qrBitmap4, New PointF(xPos4, yPos))
                End If

                ' SRNO5
                If Not IsDBNull(row.Item("srno5")) Then
                    Dim printSrno5 As String = row.Item("srno5").ToString()
                    Dim qrcode5 As New BarcodeWriter
                    qrcode5.Format = BarcodeFormat.QR_CODE
                    Dim qrBitmap5 As Bitmap = qrcode5.Write(printSrno5)
                    e.Graphics.DrawImage(qrBitmap5, New PointF(xPos5, yPos))
                End If

                yPos += qrBitmap1.Height + Math.Max(qrcode21YMargin, qrcode22YMargin)
            Next
        End If
    End Sub
End Class